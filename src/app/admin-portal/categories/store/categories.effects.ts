...
import { REQUEST_LOAD_CATEGORIES, LoadCategories, CategoriesError, CategoryAction,
  ...
  CategoryCreateSuccess, REQUEST_LOAD_CATEGORY, CategoryLoadSuccess, ResetEditingCategory, REQUEST_EDIT_CATEGORY, CategoryEditSuccess } from './categories.actions';
...
/**
 * The categories effect class to do some asynchronous actions. Loading of categories, deleting categories, creating categories and updating.
 *
 * @export
...
export class CategoriesEffect {
  ...
  $requestLoadCategoriesEffect = this.actions$
    .ofType(REQUEST_LOAD_CATEGORIES)
  ...
  $requestDeleteCategoryEffect = this.actions$
    .ofType(REQUEST_DELETE_CATEGORY)
    .map((action: CategoryAction) => action.payload)
    .mergeMap((id: string) => {
  ...
  $requestCreateCategoryEffect = this.actions$
    .ofType(REQUEST_CREATE_CATEGORY)
    .map((action: CategoryAction) => action.payload)
  ...
  $requestLoadCategoryEffect = this.actions$
    .ofType(REQUEST_LOAD_CATEGORY)
    .map((action: CategoryAction) => action.payload)
    .mergeMap((id: string) => {
      this.store.dispatch(new ResetEditingCategory());
      return this.categoriesService.loadCategory(id)
        .map((response: DataResponse<CategoryModel>) => {
          if (response.data) {
  ...
  $requestEditCategoryEffect = this.actions$
    .ofType(REQUEST_EDIT_CATEGORY)
    .map((action: CategoryAction) => action.payload)
    .mergeMap((category: CategoryModel) => {
      return this.categoriesService.editCategory(category)
        .map((response: DataResponse<CategoryModel>) => {
          if (response.data) {
            this.router.navigate(['/admin/category']);
